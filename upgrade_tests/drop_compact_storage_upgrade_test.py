...
class TestDropCompactStorage(Tester):
    def test_drop_compact_storage(self):
        """
        Test to verify that dropping compact storage is not possible prior running `nodetool upgradesstables`. The current
        solution and test take care only about the local sstables. Global check of the sstables will be added as part of
        CASSANDRA-15897
        @CASSANDRA-16063
        """
